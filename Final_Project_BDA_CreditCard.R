...
########################Data exploration##################################
#cor.credit_card <- cor(credit_card) , method = c("limit_bal","sex","education"))
#library(corrplot)
#corrplot(cor.credit_card, order = "hclust", tl.col = "black", tl.srt = 45)
...
library(neuralnet)
library(caret)
#credit_card <- credit_card[ , -c(2, 3, 4)]
str(credit_card)
...
pred.prob <- predict(pcs.df.nb, newdata = valid.df, type = "raw")
pred.prob
## predict class membership
